...
async function handleRealAction(action: string, accountId: string, campaignId?: string, budget?: number) {
...
export const googleAdsTool = createTool({
  ...
  execute: async (inputData) => {
    ...
    try {
      return await handleRealAction(action, accountId, campaignId, budget);
    } catch (error) {
      return {
        success: false,
